...
// Stack (Riwayat Tindakan Medis)
RiwayatTindakan riwayat[MAX];
int topStack = -1;
...
void enqueueAntrian() {
    ...
    if (isQueueFull()) {
        cout << "\n   [!] Queue overflow! Antrian penuh! Kapasitas maksimum (" << MAX << ") tercapai." << endl;
        tekanEnter();
        return;
    }
    ...
    Hewan* tujuan = antrian + rear;
    *tujuan = *sumber;

    int totalAntrian = rear - front + 1;
    cout << "\n   [OK] Hewan " << sumber->nama << " (ID: " << sumber->id
         << ") berhasil masuk antrian!" << endl;
    cout << "   Posisi dalam antrian: " << totalAntrian << endl;
    cout << "   Jumlah antrian saat ini: " << totalAntrian << endl;
    tekanEnter();
}
...
void dequeueAntrian() {
    ...
    Hewan* pasienDipanggil = antrian + front;

    // Tampilkan nama & ID hewan yang dipanggil
    cout << "\n   ============================================" << endl;
    ...
    int sisaAntrian = isQueueEmpty() ? 0 : (rear - front + 1);
    cout << "\n   Sisa antrian: " << sisaAntrian << " pasien" << endl;
    tekanEnter();
}
...
void tampilSemuaAntrian() {
    ...
    int totalAntrian = rear - front + 1;
    cout << "\n   Antrian dari FRONT ke REAR (" << totalAntrian << " pasien):\n" << endl;
    ...
    Hewan* ptrBase = antrian;
    int nomor = 1;
    for (int i = front; i <= rear; i++) {
        Hewan* ptr = ptrBase + i; 
        string status = "";
        if (i == front) status = "[FRONT]";
        if (i == rear) {
            if (i == front) status = "[F & R]";
            else status = "[REAR]";
        }
...
void pushRiwayat(RiwayatTindakan* stack, int* top) {
    CLEAR;
    cout << "\n   +========================================================+" << endl;
    ...
    if (isStackFull()) {
        cout << "\n   [!] OVERFLOW: Stack riwayat penuh! Kapasitas maksimum (" << MAX << ") tercapai." << endl;
        tekanEnter();
        return;
    }
    ...
    // Push ke stack menggunakan pointer
    (*top)++;
    RiwayatTindakan* ptrTop = stack + (*top);
    ptrTop->idHewan = hewanPtr->id;
    ptrTop->namaHewan = hewanPtr->nama;
    ptrTop->jenisHewan = hewanPtr->jenis;
    ptrTop->tindakan = tindakan;
    ptrTop->waktu = getWaktuSekarang();
...
void popRiwayat() {
    ...
    // Ambil data tindakan terakhir (top)
    RiwayatTindakan* ptrTop = riwayat + topStack;
    ...
    if (topStack >= 0)
        cout << "   Sisa riwayat: " << topStack + 1 << " tindakan" << endl;
    else
...
void tampilSemuaRiwayat() {
    ...
    cout << "\n   Riwayat dari TOP ke BOTTOM (" << topStack + 1 << " tindakan):\n" << endl;
    ...
    // Iterasi dari top ke bottom menggunakan pointer aritmatika
    RiwayatTindakan* ptrBase = riwayat;
    int nomor = 1;
    for (int i = topStack; i >= 0; i--) {
        RiwayatTindakan* ptr = ptrBase + i;
        string posisi = "";
        if (i == topStack) posisi = "TOP";
        if (i == 0) {
            if (i == topStack) posisi = "T&B";
            else posisi = "BTM";
        }

        cout << "   | " << setw(4)  << nomor
             << "| " << setw(6)  << ptr->idHewan
             << "| " << setw(16) << ptr->namaHewan
             << "| " << setw(12) << ptr->jenisHewan
             << "| " << setw(20) << ptr->tindakan
             << "| " << setw(15) << ptr->waktu
             << "| " << setw(4)  << posisi
             << "|" << endl;
        nomor++;
...
int main() {
    ...
    while (true) {
        ...
        switch (pilihan) {
            ...
            case 11: pushRiwayat(riwayat, &topStack); break;
